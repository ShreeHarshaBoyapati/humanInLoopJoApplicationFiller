...
### Checkpoint 4 — Extension: background WS owner + cache invalidation (DONE)
...
- **Actually shipped:** see implementation notes in §13. SW owns one socket; pure-function handler dispatches IDB patches + invalidations; each section component listens for `RESOURCE_CHANGED` and refetches on its own resource. Patch in place for `update`/`setActive`; invalidate for `create`/`branch`/`delete`; count-only `related` is patched in place.
...
### Checkpoint 4 — implementation notes (added when Checkpoint 4 landed)

- **Why the background service worker (not the side panel) holds the WebSocket.** MV3 service workers can be terminated by Chrome after ~30s of inactivity and wake on the next extension event. The side panel has the opposite lifecycle: it spawns when the user opens it and dies when the user closes it. Putting the WS in the side panel would mean (a) the user must open the panel to receive sync, (b) closing the panel disconnects the socket and the panel misses any update that happened while it was closed. The SW is the long-lived owner; the side panel is a leaf that joins when it mounts and leaves when it unmounts. Same model as the web's React provider, just in a different process.

- **Why `chrome.runtime.sendMessage` and not a per-panel WebSocket.** Chrome only allows one side panel per window and the panel re-renders the same UI tree regardless. A second WebSocket per panel would double the backend's `peerCount` per user for no benefit and would duplicate every cache-patch operation (the same `RESOURCE_CHANGED` event arriving over two sockets would be applied twice). The SW opens one socket, the IDB patch is applied once at the SW level, and the panel just receives a small `RESOURCE_CHANGED` message that triggers a refetch. The pattern matches the existing `LOGOUT_TRIGGERED` message in `apps/extension/src/routes/__root.tsx`.

- **Why `chrome.runtime.sendMessage` is wrapped in `try/catch` + `.catch(noop)`.** When the side panel is closed, `sendMessage` rejects with "Could not establish connection. Receiving end does not exist." If we let that error propagate, Chrome logs it as a noisy extension error on every event. We swallow it silently. Per the §11 "no noise" rule, the SW does its job (applies the IDB patch, broadcasts if a panel is listening) and the absence of a panel is a no-op, not a failure.

- **Why the JWT is read from `chrome.storage.session` and not from a new `chrome.storage.local` key.** The existing auth sync flow (`apps/extension/src/content/auth-sync.ts` + the axios interceptor in `apps/extension/src/background/index.ts`) already writes the JWT to `chrome.storage.session` under `AUTH_STORAGE_KEY`. Reusing that single source of truth means the SW's owner doesn't need a second token-storage path or a token-rotation listener beyond `chrome.storage.onChanged`. Login from the web app posts a `SYNC_AUTH_FROM_WEB` message → background writes the session storage → the SW's `onChanged` listener sees the new token and starts the socket. Logout removes the storage → listener sees it and stops the socket. One source, one listener, no drift.

- **Why the owner (`RealtimeOwner`) watches `chrome.storage.onChanged` instead of polling.** A polling loop would race with the actual login/logout handlers and could read a stale token for a few seconds after the user logs out, briefly holding a socket under a dead user's room. The `onChanged` event fires synchronously on storage mutations and is the exact same mechanism the existing `__root.tsx` `clearAllCache()`-on-token-change uses — same pattern, same correctness guarantees.

- **Why the owner patches IndexedDB before sending the message to the panel.** If the panel refetched on a `RESOURCE_CHANGED` for an `update` / `setActive` event, the panel would read its (stale) cache, then re-render with the old data, then dispatch `RESET` + refetch, then re-render with the fresh data — a visible flash. By patching IDB at the SW level _first_, the panel's refetch on `create`/`branch`/`delete` reads the just-patched page, and the next render is the patched data with no flash. The `update`/`setActive` events don't trigger a panel refetch at all (the SW already patched in place). The visible result is the same "in-place update with no spinner" experience the web app has.

- **Why `applyRealtimeEventToCache` is a pure function over a 11-function `deps` object (not a React hook or an IDB-aware module).** The handler is the part that turns events into IDB mutations. Keeping it free of `chrome.*` and `idb` means (a) it can be tested in isolation with a vitest mock, (b) the side-panel component (and any future owner) can wire up its own `deps` and reuse the exact same dispatch table, (c) the SW's `RealtimeOwner` and the panel's listener are decoupled. The 11 functions (3 patch-by-id, 2 patch-count, 3 scope-specific clears, 3 full invalidates) are the smallest set that covers the 9-cell handler table from §13.

- **Why each `*-section.tsx` component registers its own `chrome.runtime.onMessage` listener (not a single `__root.tsx` listener or a zustand `cacheVersion` counter).** Each section is its own React tree with its own `useReducer`/`getPage`/refetch wiring. Putting the listener in the section means (a) the listener is automatically removed when the section unmounts (the `useEffect` cleanup), (b) the section has direct access to `dispatch` and `fetchPersonas` without prop-drilling or a global store, (c) there's no dependency between `__root.tsx` and the section components for sync. The trade-off is three listeners instead of one, but each is ~15 lines and they only fire on a real-time event (rare). A `__root.tsx` listener would have to broadcast back to sections somehow; a zustand counter would force a re-render of every consumer, including non-list views.

- **Why the per-section listener filters on `action === 'update' | 'setActive'` and returns early.** The SW has already patched the cache in place for those events. The panel's `getPage` will return the fresh data. If the listener also called `dispatch({ type: 'RESET' })` + refetch, the user would see a flash. So the listener only refetches on `create` / `branch` / `delete`, where the new row's full content isn't in the event and a refetch is the only correct move.

- **Why the IDB patch helpers (`patchPersonaInPages`, etc.) use a cursor with a `mutated` boolean and `break` after a single `cursor.update`, not a `for-of` over `getAll`.** The persona store is keyed by `${token}:${search}:${page}` and a single row is in at most one cached page (paginated, deduplicated list). A `break` after the first successful patch is the early-exit that doesn't miss any work. The `mutated` flag is needed because `Array.prototype.map` always returns a new array reference, so a reference equality check (`nextItems !== value.items`) can't tell us if anything actually changed — a `let mutated = false; if (matched) mutated = true;` inside the map is the only way. The IDB cursor's `update` writes the current entry and leaves the cursor at the same position; `break` then exits the while loop cleanly. Per the §11 "no comments" rule, none of this is commented in code — it lives here.

- **Why `clearVersionsForPersona(personaId)` walks the `resumes` store to find resumeIds instead of using a stored index.** The IDB schema doesn't have a `personaId → resumeIds` index (the `resumes` store is keyed by `${token}:${personaId}:${search}:${page}`, so a key-prefix scan can find every page for a given persona, but the resume ids themselves live inside each page's `items` array). The walk is O(num persona pages × items per page), which for the app's pagination size (10 per page, max ~10 pages cached) is at most ~100 cursor reads. Local-only, no backend call. The result is the set of resumeIds, and we call the existing `invalidateVersionsForResume(resumeId)` for each. Two passes, no new index, no migration of the IDB `DB_VERSION`.

- **Why `RealtimeClient` is duplicated in the extension instead of moved to `@repo/utils`.** The two copies are ~50 lines and the contract is fixed ("open WS, exponential backoff on close, forward messages"). Moving it to `@repo/utils` would add a workspace dependency and a new export surface for a file that's already in two places. Drift risk is low because the only intentional difference is `resolveRealtimeUrl()` reading `VITE_EXT_WS_URL` (env) instead of `VITE_WS_URL`. If the contract ever grows (e.g. add ping-pong in Checkpoint 7), we move it then.

- **Why `chrome.runtime.onSuspend?.addListener(realtimeOwner.stop)` is optional.** The optional chain (`?.`) covers the case where the SW is being torn down by Chrome without firing `onSuspend` (e.g. an abrupt kill). MV3 best practice is to do best-effort cleanup in `onSuspend` and accept that not every termination goes through it. The owner itself stores the storage listener and the client, so on the next SW startup we rebuild both from scratch via `realtimeOwner.start()`.

- **Why we don't re-fetch the side panel automatically on `hello` after a reconnect.** Per the §8 "no replay in v1" decision, the side panel does nothing on socket reconnect. If a user did a persona mutation while the side panel was disconnected, the SW will see the next mutation and the patch will arrive; the panel will see the cache updated on its next read (a refetch on a `create`/`branch`/`delete`, or in-place for `update`/`setActive`). If the user does nothing on the backend between disconnect and reconnect, the panel keeps its stale-but-correct data until the user makes a change in the web app. This is the same trade-off the web made in Checkpoint 3.

- **Stop and verify (manual).** Start the backend, log in on the web app, open the extension side panel. In a terminal: `cd apps/backend && node -e 'const W=require("ws");const ws=new W("ws://localhost:8000/ws",["<token>"]);ws.on("open",()=>console.log("OPEN"));ws.on("message",m=>console.log("MSG",m.toString()));ws.on("close",(c,r)=>console.log("CLOSE",c,r&&r.toString()));ws.on("error",e=>console.log("ERR",e.message));'`. From the web app: create a persona. The terminal shows `MSG {"type":"resource.changed",...}` and the side panel's persona list updates without a refresh. Edit the persona's title — the side panel's row updates in place. Set a version as active — the side panel's persona, resume, and version rows all flip `active` in place. Delete a persona — the side panel refetches. Kill the backend (`Ctrl+C`) and restart it; the SW reconnects with 1s → 30s exponential backoff. Open the extension DevTools (chrome://extensions → service worker → "Inspect views: service worker") to see the `[ws] connected user:<id> peerCount=<n>` logs.

- **Manual UI test cases (the contract you will run in the browser):**
  1. **Create persona in web, side panel updates.** Open side panel on the personas page; create a persona in the web app; side panel refetches and shows the new row.
  2. **Edit persona title in web, side panel row updates in place (no spinner).** Edit a title in the web app; side panel's row text changes without a loading flash.
  3. **Set version active in web, side panel's three rows all flip.** Set a version active in the web app; side panel's persona `active`, resume `active`, and version `active` all flip in place.
  4. **Delete resume in web, side panel refetches; persona's `versionsCount` decrements without a refetch.** Delete a resume; side panel's resume list refetches; the parent persona's "N resumes" text updates (in-place via the count patch) without a refetch of the persona list.
  5. **Delete persona in web, side panel refetches; resumes + versions cleared.** Delete a persona; side panel's persona list refetches; any cached resumes for that persona are cleared; any cached versions for those resumes are cleared. The next time the user opens a different persona's resume list, the data is fresh from the backend.
  6. **Kill backend, restart, side panel recovers.** `Ctrl+C` the backend, restart it. The SW's `RealtimeClient` reconnects with backoff. Open the side panel DevTools to watch the `[ws] connecting` / `[ws] open` log lines. The next mutation flows through.
  7. **Login from web, side panel becomes live.** Log in on the web app; the `SYNC_AUTH_FROM_WEB` writes the JWT to `chrome.storage.session`; the SW's `onChanged` listener starts the socket; the side panel begins receiving events. (If the side panel was open before login, the `RESOURCE_CHANGED` messages start arriving without a panel reload.)
  8. **Logout from web, side panel stops receiving events.** Log out from the web app; the `AUTH_LOGOUT` clears `chrome.storage.session`; the SW's `onChanged` listener stops the socket. The next mutation on the web does not arrive in the side panel.
  9. **Open side panel before login, then login — the side panel starts receiving without a remount.** The per-section `useEffect` adds the listener on mount, and the WS starts (via the SW's storage listener) when the token appears. No need to reopen the side panel.
